...
- Генерирует список действий которые будут исполнены скриптом `main.cs` `IAction` при самом первом запуске ОС после `winpe` (установка програм, пользователей, кастомные команды). Плагин перобразовывает `YAML` инструкции для `main.cs` в JSON.
